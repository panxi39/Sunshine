...
namespace platf {
   ...
   *
   * Some setups (e.g. running as a user in the `video` group without an active seat) get
   * an implicit DRM master handle when opening `/dev/dri/cardN`. Holding master prevents
   * the compositor from becoming master again on VT switches and breaks capture. This
   * helper opens the card node, detects whether the resulting fd is master, and if so
   * calls `drmDropMaster` and re-verifies the state before returning the fd.
   *
